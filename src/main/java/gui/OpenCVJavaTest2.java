...
public class OpenCVJavaTest2 {
    static final String originalImgPath = ".\\src\\main\\resources\\test_img\\erweima2.jpg";
...
//        ImageViewer imageViewer = new ImageViewer(src_all, "图片预览");
//        imageViewer.imshow();
        /***
        ...
        if (markContours.size() < 3) {
            ...
            for (int i = 0; i < markContours.size() - 2; i++) {
                List<MatOfPoint> threePointList = new ArrayList<MatOfPoint>();
    ...
    public static void capture(List<MatOfPoint> contours, Mat src, String idx) {
      ...
      /*  if (Math.max(ca[0],cb[0])/Math.min(ca[0],cb[0]) > 1.5 || Math.max(ca[1],cb[1])/Math.min(ca[1],cb[1])>1.3){
        ...
        //imshow("高斯模糊",dstRoi);
        // 展示处理结果
        ImageViewer imageViewer = new ImageViewer(dstRoi, "图片预览");
        imageViewer.imshow();
    }
